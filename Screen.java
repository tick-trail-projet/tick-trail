...
public class Screen {
	...
	void updateSeats(ArrayList<Integer> seat){                    // jai essaye de parcourire en forme de tableau ou list les place jes asi pas si c juste 
		int counter=1;
	...
	void printScreenDetails(){
		...
		for(int i=0;i<rows;i++)
			for(int j=0;j<columns;j++){
				if (Seatavailable[i][j]==0)                              // je pense que le conteur marche je lai essayer apart et ca marcher des fois oui des fois non  hhhhh
	...
	ArrayList<Integer> checkAvailability(String ScreenTime, int SeatClass, int noOfSeats){
		...
		for(int i=0;i<columns;i++){
			if(count>noOfSeats)                                        // ca marche mais quand c zero ca bug fort 
				break;
	...
	void showAvailableSeats(int index){
		...
		int count=0;
		for(int i=0;i<columns;i++){
			if(Seatavailable[0][i]==0)                // meme truc ca marche mais je suis pas sur si le code est parfait 
				count++;
		}
